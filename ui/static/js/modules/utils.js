...
window.AppUtils = (function() {
    return {
        ...
        notifyAgentDone(tab) {
            const isBackground = document.hidden || this.activeChatTab !== tab.tab_id;
            if (!isBackground) return;
            // Play sound
            this.playNotificationSound();
            // Browser notification (if permission granted)
            if ('Notification' in window && Notification.permission === 'granted') {
                const label = tab.label || 'Chat';
                new Notification('AutoResearch — ' + label, {
                    body: 'Agent finished streaming',
                    icon: '/static/favicon.ico',
                    tag: 'ar-chat-' + tab.tab_id,
                });
            }
        },
    };
