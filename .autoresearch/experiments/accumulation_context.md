...
1. **MOUTH_YAWN sprite** (7×5 px) — широко открытый рот для зевания. Показывается во время stretch фаз 1-2.
2. **renderExpressionOverlays()** — визуальные overlay-эффекты: blush, sweat drop, tear, angry vein.
...
1. **EVENT_JUDGE** — New event type `judge_verdict` added to research.py event constants.
2. **_run_judge()** — Method on ResearchRunner that runs `ExperimentJudge.evaluate_all()` after each successful experiment. Non-fatal (catches exceptions, logs warning).
3. **Auto-judge in run_loop()** — After `EVENT_EXP_END`, if status is "success", calls `_run_judge()` and emits `EVENT_JUDGE` with consensus, score, and per-profile verdicts.
4. **Persistence** — Judge verdicts saved to `.autoresearch/experiments/judge_{n}_all.json`.
5. **Server logging** — `judge_verdict` event handler in server.py logs consensus with per-profile breakdown.
6. **Lab UI live log** — Judge verdicts appear in live log with color-coded icon (green=KEEP, red=DISCARD, amber=REVIEW).
7. **Cat reactions** — Cat reacts to judge verdicts with expressions and Russian speech.
...
1. **Live thinking preview** — During agent thinking phase, when `_thinkingBuffer` accumulates content, a live preview is shown in a collapsible block with blinking cursor.

2. **Collapsible toggle** — User can collapse/expand the live thinking preview (respects `settings.showThinking`).

3. **Character count** — Shows character count of thinking buffer.

4. **CSS animations** — Blinking cursor at end of text and in header toggle.

5. **Graceful fallback** — When buffer is empty, shows standard "THINKING..." with animated dots (unchanged).
...
1. **AGENT_CONTEXT_SKILL_TIPS** — 10 keyword-groups with relevant slash-commands and Russian tips from cat.

2. **Extended analyzeAgentResponse()** — priority keyword-based analysis of agent content.

3. **getContextualSkillSuggestion(messages)** — analyzes last 6 messages, finds follow-up patterns.

4. **setChatContext(messages)** — method for idle tips timer context update.

5. **Contextual idle tips** — cat suggests skill based on recent messages.
...
- ui/static/modules/cat.js — AGENT_CONTEXT_SKILL_TIPS, _getContextualSkillSuggestion(), setChatContext()

- ui/static/js/modules/chat.js — getContextualSkillSuggestion after streaming, setChatContext on tab switch
...
1. **Ctrl+Up shortcut** — Нажатие Ctrl+Up (Cmd+Up на Mac) мгновенно входит в режим редактирования последнего сообщения пользователя.
2. **Справочник горячих клавиш** — Добавлено "Ctrl+Up: Edit last user message" в CHAT секцию.
3. **Welcome screen** — Добавлена подсказка Ctrl+Up.

### Files Modified

- `ui/static/js/modules/chat.js` — Ctrl+Up handler
- `ui/static/js/app.js` — keyboardShortcuts
- `ui/static/templates/chat-section.js` — welcome screen
