# AutoResearch Experiment 113/1106
...
- В Research lab - RUN experiment выбор проекта/папки (exp58: file browser + preflight check). Осталось: интерактивное анкетирование если файлы/настройки отсутствуют
...
- ✦ TEMPLATES В чате при раскрытии подсказки лучше пускай будут по скиллам /speckit типа - "Начать описание фичи" при нажатии вводится команда /speckit.specify (но не точно, сделай как лучше, возможно другую реализацию идей, но текущий вариант ✦ TEMPLATES не катируется.
- РЕАКЦИИ (thumbs up/down) — реализовано в exp94 (кнопки) и exp97 (фидбек агенту). При клике на реакцию она ставится в очередь и автоматически препендится к следующему сообщению как контекст для агента. Badge-индикатор на кнопке SEND показывает количество ожидающих реакций.
- Добавить чату дашборд со сбором различной статистики активности, потраченных токенов и т.д
- Добавь всяких прикольных фраз и просто под разные ситуации коту из Warcraft 3 типа "Нам нужно больше золота"
...
```bash
git add -A
git commit -m "exp #113: <краткое описание что изменено>"
```
