...
**AutoResearch** расширяет идею автономного AI-исследования на ЛЮБОЙ проект:

| Оригинал (karpathy/autoresearch) | AutoResearch |
|----------------------------------|---------------|
| ML training (nanochat) | **Любой проект** (Python, JS, Go, ...) |
| Только `train.py` | **Любые файлы** — полный контроль |
| Одна метрика (`val_bpb`) | **Любые метрики** — гибкость |
| GPU, training cycles | **CPU, любые операции** |
| Специфическая задача | **Универсальность** |

### Что делает AutoResearch?
...
| Характеристика | karpathy/autoresearch | AutoResearch |
|----------------|----------------------|---------------|
| **Область применения** | ML training (nanochat) | Любой проект |
| **Файловые ограничения** | Только `train.py` | Любые файлы проекта |
| **Метрики** | Одна (`val_bpb`) | Любые (гибко настраиваются) |
| **Среда выполнения** | GPU, training | CPU, любые операции |
| **Платформа** | Специфическая | Кроссплатформенная |
| **Накопление знаний** | В рамках сессии | Между запусками (файлы) |
| **Настройка** | Ручная (`program.md`) | Интерактивный опросник |
| **Контекст** | Инструкции | Инструкции + предыдущие эксперименты |

**Что сохранено:**
- ✅ Автономность — агент работает самостоятельно
- ✅ Итеративное улучшение — каждое изменение оценивается
- ✅ Human-in-the-loop — человек направляет через цели/ограничения
- ✅ Fail-fast — плохие изменения отбрасываются

**Что улучшено:**
- ✨ Универсальность — любой язык, любой тип проекта
- 🌐 Кроссплатформенность — Windows, Linux, macOS
- 📚 Постоянное накопление — контекст сохраняется между запусками
- 🎨 Гибкая настройка — опросник адаптирует под проект
- 🔧 Удобный CLI — простой запуск для любого проекта
